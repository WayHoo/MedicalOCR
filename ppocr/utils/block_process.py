...
def calc_block_angle(dt_boxes, rec_res):
    ...
    for box in dt_boxes:
        ...
        print('angle=%.2f, ratio=%.2f, text=%s, score=%.2f, box=%s' % (angle, ratio, text, score, '...'))
...
def test_sheet_extract(img, dt_boxes, rec_res):
    ...
    head_words = {'序号', 'No', '代号', '项目代号', '项目名称', '项目', '检验项目', '英文对照', '英文代码',
                  '英文', '项目简称', '结果', '项目结果', '结果浓度', '测定结果', '检测结果', '标志',
                  '异常提示结果', '提示', '结果提示', '结果描述', '报警', '单位', '参考范围', '参考值',
                  '参考区间', '方法学', '检验方法', '实验方法'}
    # 提取化验栏标题并判断单栏或双栏
    diff_word_cnt = 0.0
    total_word_cnt = 0.0
    ...
    for k in text_map:
        if k not in head_words:
            other_boxes.extend(text_map[k])
