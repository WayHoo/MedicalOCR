...
def init_model(config, model, logger, optimizer=None, lr_scheduler=None):
    """
    load README.md from checkpoint or pretrained_model
    """
    ...
    if checkpoints:
        ...
        for idx, pretrained in enumerate(pretrained_model):
            ...
            logger.info("load pretrained README.md from {}".format(
                pretrained_model))
    else:
...
def save_model(net,
               ...
               **kwargs):
    """
    save README.md to the target path
    """
    ...
    if is_best:
        logger.info('save best README.md is to {}'.format(model_prefix))
    else:
        logger.info("save README.md in {}".format(model_prefix))
