...
class TableRecognizer(object):
    def __init__(self, args, img, img_name, dt_boxes, rec_res):
        """
        初始化
        :param args: 配置参数
        :param img: 化验单图像
    ...
    def get_same_line_head_boxes(self):
        ...
        for tmp_key in same_line_boxes:
            if tmp_key not in visited:
                ...
                line = sorted(line, key=lambda k: self.text_boxes[k].box[0][0])
                lines.append(line)
        return lines
    ...
    def get_head_line_f_1(self, box_keys):
        ...
        if len(box_keys) == 1:
            ...
            logger.info("head line ignored, text: {}".format(self.text_boxes[key].text))
        ...
        # TODO: 化验单栏数判断阈值设定
        if sub_table_ratio - math.floor(sub_table_ratio) >= self.args.sub_table_thresh:
        ...
        if avg_seg_ratio >= self.args.head_word_seg_thresh:
    ...
    def split_horizon_lines(self, box_keys, k, b):
        ...
        """
        box_dis = []
        thresh = self.args.horizon_line_split_thresh
        for key in box_keys:
            box = self.text_boxes[key].box
