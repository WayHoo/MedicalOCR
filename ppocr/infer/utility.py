...
def parse_args():
    ...
    parser.add_argument("--image_dir", type=str, default='./doc/imgs/test_sheets/batch_001/test_sheet (7).jpg')
...
def create_predictor(args, mode, logger):
    ...
    if model_dir is None:
        logger.info("not find {} README.md file path {}".format(mode, model_dir))
        sys.exit(0)
    model_file_path = model_dir + "/inference.pdmodel"
    params_file_path = model_dir + "/inference.pdiparams"
    if not os.path.exists(model_file_path):
        logger.info("not find README.md file path {}".format(model_file_path))
        sys.exit(0)
